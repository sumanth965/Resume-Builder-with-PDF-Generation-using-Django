...
- **Global scope**: `D:/Documentsss/resume_project/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Documentsss/resume_project/.gemini/notes/`)
...
2. Glob `D:/Documentsss/resume_project/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (D:/Documentsss/resume_project/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Documentsss/resume_project/.gemini/notes/` — works in any directory
