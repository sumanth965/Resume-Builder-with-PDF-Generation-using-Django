...
```bash
# Get state snapshot
node "D:/Documentsss/resume_project/.gemini/get-shit-done/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```bash
node "D:/Documentsss/resume_project/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs: defer incomplete Phase {src} items to backlog"
```
